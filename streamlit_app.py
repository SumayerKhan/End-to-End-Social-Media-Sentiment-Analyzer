...
I analyze 38,000+ Reddit discussions to help you understand what people think about tech products.
...
def display_source_card(post: Dict[str, Any], index: int):
    ...
    <div class="source-card">
        ...
        <div class="source-card-meta">
            r/{subreddit} • {score} upvotes • {similarity:.1f}% relevant • {sentiment_html}
        </div>
        <a href="{reddit_url}" target="_blank" class="source-card-link">View on Reddit →</a>
    </div>
    """, unsafe_allow_html=True)
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # Subreddit filter
        subreddit_options = ["All Subreddits", "iphone", "apple", "samsung", "android",
                            "laptops", "buildapc", "pcmasterrace", "gaming",
                            "mechanicalkeyboards", "headphones"]
        selected_subreddit_display = st.selectbox(
            "Subreddit",
            options=subreddit_options,
            index=0
        )
        st.session_state.selected_subreddit = None if selected_subreddit_display == "All Subreddits" else selected_subreddit_display
